...
export default function ContributionPayButton({ email, amount, label = "Pay Contribution" }: Props) {
  ...
  async function handleClick() {
    ...
    try {
      ...
      console.error("[ContributionPayButton] SDK load failed:", err);
    ...
    try {
      const res = await fetch("/api/squad/init-payment", {
        ...
        body: JSON.stringify({ email, amount, payType: "contribution" }),
      });
      ...
      console.error("[ContributionPayButton] init-payment:", err);
    ...
    sessionStorage.setItem("squad_tx_type", "contribution");
    ...
    try {
      ...
      console.error("[ContributionPayButton] modal open failed:", err);
