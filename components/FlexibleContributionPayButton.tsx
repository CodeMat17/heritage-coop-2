...
export default function FlexibleContributionPayButton({ email, onSuccess, onClose }: Props) {
  ...
  async function handlePay() {
    ...
    if (!email || !amountNaira || amountNaira < MIN_AMOUNT) {
          ...
          for (let attempt = 0; attempt <= delays.length; attempt++) {
            ...
            try {
              const verifyRes = await fetch(
                `/api/squad/verify-contributions/${data.transaction_ref}`,
                {
                  method: "POST",
                  headers: { "Content-Type": "application/json" },
              ...
              console.error("[verify-contributions] failed after retries:", verifyData?.message);
            } catch (err) {
              if (attempt < delays.length) continue;
